...
WHERE b.bilding_id=$1;`;
  ...
  try {
    ...
    let mailOptions = {
      ...
      subject,
      text: 'Error en el email.',
      html: contentEmailInvoice( results.rows[0].id, results.rows[0].nombre, results.rows[0].name_shopping, moment().tz(zonaHoraria).format('YYYY/MM/DD hh:mm:ss a'), results.rows[0].type_payment,  results.rows[0].mount_discount, results.rows[0].propina, results.rows[0].subtotal, results.rows[0].total, line, results.rows[0].type_discount, results.rows[0].order_id, results.rows[0].payment_id, results.rows[0].iva, table_id )
    };
